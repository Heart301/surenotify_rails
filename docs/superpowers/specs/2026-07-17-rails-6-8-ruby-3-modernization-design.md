...
4. **版本**：升為 1.0.0
5. **附件**：移除附件支援（原本就是壞的），README 註明不支援；
   待確認 Surenotify API 附件格式後再正確實作
...
- 保留現有訊息組裝行為：`fromName` / `fromAddress` / `recipients` / `subject` /
  `content`（HTML）、cc/bcc、空值移除
...
- README 更新：支援版本說明（Rails 6.1+ / Ruby 3.0+）、註明不支援附件
...
- API 回傳非 2xx 時 `deliver!` 不設定 message_id，直接回傳 response（維持原行為）
